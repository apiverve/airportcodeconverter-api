...
class AirportcodeconverterValidationException implements Exception {
  final List<String> errors;

  AirportcodeconverterValidationException(this.errors);

  @override
  String toString() => 'AirportcodeconverterValidationException: ${errors.join("; ")}';
}
...
/// Airport Code Converter API client.
///
/// For more information, visit: https://apiverve.com/marketplace/airportcodeconverter?utm_source=dart&utm_medium=readme
///
/// Parameters:
/// * [code] (required) - The airport code to convert (IATA: 3 letters, ICAO: 4 letters)
class AirportcodeconverterClient {
  final String apiKey;
  ...
  AirportcodeconverterClient(this.apiKey, {
    this.baseUrl = 'https://api.apiverve.com/v1/airportcodeconverter',
    http.Client? httpClient,
  }) : _httpClient = httpClient ?? http.Client();

  /// Validates parameters against defined rules.
  /// Throws [AirportcodeconverterValidationException] if validation fails.
  void _validateParams(Map<String, dynamic> params) {
    ...
    if (errors.isNotEmpty) {
      throw AirportcodeconverterValidationException(errors);
    }
  }

  /// Execute a request to the Airport Code Converter API.
  ///
  /// Parameters are validated before sending the request.
  Future<AirportcodeconverterResponse> execute(Map<String, dynamic> params) async {
    // Validate parameters
    _validateParams(params);
    if (apiKey.isEmpty) {
      throw AirportcodeconverterException('API key is required. Get your API key at: https://apiverve.com');
    }
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        return AirportcodeconverterResponse.fromJson(json);
      } else if (response.statusCode == 401) {
        throw AirportcodeconverterException('Invalid API key');
      } else if (response.statusCode == 404) {
        throw AirportcodeconverterException('Resource not found');
      } else {
        throw AirportcodeconverterException('API error: ${response.statusCode}');
      }
    } catch (e) {
      if (e is AirportcodeconverterException) rethrow;
      throw AirportcodeconverterException('Request failed: $e');
    }
  }
...
/// Exception thrown by the Airport Code Converter API client.
class AirportcodeconverterException implements Exception {
  final String message;

  AirportcodeconverterException(this.message);

  @override
  String toString() => 'AirportcodeconverterException: $message';
}
